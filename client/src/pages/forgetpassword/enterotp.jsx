...
import { verifyforgotpasswordotp, sendforgotpasswordotp } from "../../service/authservice";
...
function Enterotp() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      navigate("/resetpassword", {
        state: { resetToken: result.resetToken, email },
      });
    } catch (err) {
  ...
  const handleResendOtp = async () => {
    ...
    try {
      ...
      setInfoMessage(result.message || "A new password reset OTP has been sent.");
    } catch (err) {
  ...
  return (
    <div>
      ...
      >
        <h2 className="text-2xl font-bold text-center text-blue-800 mb-6">Enter Password Reset OTP</h2>
        ...
        <div className="flex justify-between items-center text-sm text-gray-800">
          ...
          <Link to="/forgetpassword" className="text-gray-700 hover:underline">
            Back
          </Link>
        </div>
      </form>
